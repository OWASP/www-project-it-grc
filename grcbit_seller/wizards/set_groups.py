...
class SetGroupsSeller(models.TransientModel):
    ...
    seller_admin_check = fields.Boolean(string="Admin", default=lambda x:x.get_current_groups_seller('Admin' if x.env.user.lang == 'en_US' else 'Administrador'))
    ...
    def assign_groups(self):
        ...
        for rec in self:
            ...
            if data['seller_admin_check'] == True:
                group_custom = self.base_values_seller('Admin' if self.env.user.lang == 'en_US' else 'Administrador')
                user = self.env.context.get('active_id')
                user_id = self.sudo().env['res.users'].search([('id','=', user)])
