...
class SetGroupsZt(models.TransientModel):
    ...
    control_draft  = fields.Boolean(string="Draft", default=lambda x:x.get_current_groups_control('Draft'))
    control_design  = fields.Boolean(string="Design", default=lambda x:x.get_current_groups_control('Design'))
    control_implementation  = fields.Boolean(string="Implementation", default=lambda x:x.get_current_groups_control('Implementation'))
    control_approval   = fields.Boolean(string="Approval", default=lambda x:x.get_current_groups_control('Approval'))
    control_reject   = fields.Boolean(string="Reject", default=lambda x:x.get_current_groups_control('Reject'))
    control_design_evaluation = fields.Boolean(string="Design Evaluation", default=lambda x:x.get_current_groups_control('Design Evaluation') )
    control_effectiveness_evaluation = fields.Boolean(string="Effectiveness Evaluation", default=lambda x:x.get_current_groups_control('Effectiveness Evaluation') )
    ...
    def assign_groups(self):
        ...
        for rec in self:
            ...
            if data['control_draft'] == True:
                group_custom = self.base_values_control('Draft')
                user = self.env.context.get('active_id')
                user_id = self.sudo().env['res.users'].search([('id','=', user)])
            ...
            if data['control_design'] == True:
                group_custom = self.base_values_control('Design')
                user = self.env.context.get('active_id')
                user_id = self.sudo().env['res.users'].search([('id','=', user)])
                group_custom.users = [(4, user)]
            else:
            ...
            if data['control_implementation'] == True:
                group_custom = self.base_values_control('Implementation')
                user = self.env.context.get('active_id')
                user_id = self.sudo().env['res.users'].search([('id','=', user)])
                group_custom.users = [(4, user)]
            else:
            ...
            if data['control_approval'] == True:
                group_custom = self.base_values_control('Approval')
                user = self.env.context.get('active_id')
                user_id = self.sudo().env['res.users'].search([('id','=', user)])
                group_custom.users = [(4, user)]
            else:
            ...
            if data['control_reject'] == True:
                group_custom = self.base_values_control('Reject')
                user = self.env.context.get('active_id')
                user_id = self.sudo().env['res.users'].search([('id','=', user)])
                group_custom.users = [(4, user)]
            else:
            ...
            if data['control_design_evaluation'] == True:
                group_custom = self.base_values_control('Design Evaluation')
                user = self.env.context.get('active_id')
                user_id = self.sudo().env['res.users'].search([('id','=', user)])
                group_custom.users = [(4, user)]
            else:
            ...
            if data['control_effectiveness_evaluation'] == True:
                group_custom = self.base_values_control('Effectiveness Evaluation')
                user = self.env.context.get('active_id')
                user_id = self.sudo().env['res.users'].search([('id','=', user)])
                group_custom.users = [(4, user)]
            else:
