...
class DataInventory(models.Model):
    ...
    rpo_value = fields.Float(string=_('RPO Value'), help="Numerical value for the recovery point objective duration.")
    rpo_unit = fields.Selection([
        ('minutes', 'Minutes'),
        ('hours', 'Hours'),
        ('days', 'Days'),
        ('weeks', 'Weeks'),
        ], string=_('RPO Unit'), default='hours', help="Time unit for the recovery point objective duration.")
    rpo_total_hours = fields.Float(string=_('RPO Total Hours'), compute='_compute_rpo_total_hours', store=True, help="Recovery point objective expressed in total hours.")
    ...
    @api.depends('rpo_value', 'rpo_unit')
    def _compute_rpo_total_hours(self):
        """Convert the RPO duration into total hours for easier reporting."""
        unit_factors = {
            'minutes': 1.0 / 60.0,
            'hours': 1.0,
            'days': 24.0,
        ...
        for record in self:
            value = record.rpo_value or 0.0
            factor = unit_factors.get(record.rpo_unit, 1.0)
            record.rpo_total_hours = value * factor
...
class DataInventoryBusinessProcess(models.Model):
    ...
    description = fields.Text(string='Description')
    #rto = fields.Float(string=_('RTO'), help="Recovery Time Objective", track_visibility='onchange')
    rto_value = fields.Float(string=_('RTO Value'), help="Numerical value for the recovery time objective duration.")
    rto_unit = fields.Selection([
        ('minutes', 'Minutes'),
        ('hours', 'Hours'),
        ('days', 'Days'),
        ('weeks', 'Weeks'),
        ], string=_('RTO Unit'), default='hours', help="Time unit for the recovery time objective duration.")
    rto_total_hours = fields.Float(string=_('RTO Total Hours'), compute='_compute_rto_total_hours', store=True, help="Recovery time objective expressed in total hours.")

    @api.depends('rto_value', 'rto_unit')
    def _compute_rto_total_hours(self):
        """Convert the RTO duration into total hours for easier reporting."""
        unit_factors = {
            'minutes': 1.0 / 60.0,
            'hours': 1.0,
            'days': 24.0,
        ...
        for record in self:
            value = record.rto_value or 0.0
            factor = unit_factors.get(record.rto_unit, 1.0)
            record.rto_total_hours = value * factor
